                               ...
                               uint32_t numPoints)
{
    ...
    else
    {
        ...
        /* In this stage the MAC operations are increased by 1 for every
           ...
           performed. Since the partial convolution starts from from firstIndex
        ...
        while (blockSize3 > 0)
        {
            ...
            while (k > 0U)
            {
                /* Reading two inputs, x[srcALen - srcBLen + 1] and x[srcALen -
                 * srcBLen + 2] of SrcA buffer and packing */
                in1 = (q15_t)*px++;
                in2 = (q15_t)*px++;
                input1 = ((q31_t)in1 & 0x0000FFFF) | ((q31_t)in2 << 16);

                /* Reading two inputs, y[srcBLen - 1] and y[srcBLen - 2] of SrcB
                 * buffer and packing */
                in1 = (q15_t)*py--;
                in2 = (q15_t)*py--;
                input2 = ((q31_t)in1 & 0x0000FFFF) | ((q31_t)in2 << 16);
                ...
                /* Reading two inputs, x[srcALen - srcBLen + 3] and x[srcALen -
                 * srcBLen + 4] of SrcA buffer and packing */
                in1 = (q15_t)*px++;
                in2 = (q15_t)*px++;
                input1 = ((q31_t)in1 & 0x0000FFFF) | ((q31_t)in2 << 16);

                /* Reading two inputs, y[srcBLen - 3] and y[srcBLen - 4] of SrcB
                 * buffer and packing */
                in1 = (q15_t)*py--;
                in2 = (q15_t)*py--;
                input2 = ((q31_t)in1 & 0x0000FFFF) | ((q31_t)in2 << 16);
