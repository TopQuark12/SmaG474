    ...
    q15_t *pSrcA, uint32_t srcALen, q15_t *pSrcB, uint32_t srcBLen, q15_t *pDst)
{
     ...
     * first part of the algorithm, the multiplications increase by one for
     * every iteration. In the second part of the algorithm, srcBLen number of
     ...
     * srcBLen should be greater than or equal to 4, to loop unroll the srcBLen
     * loop */
    if (srcBLen >= 4U)
    {
        ...
        while (blkCnt > 0U)
        {
            ...
            *pOut = (q15_t)(__SSAT(acc0 >> 15, 16));
            /* Destination pointer is updated according to the address modifier,
             * inc */
            pOut += inc;
    ...
    while (blockSize3 > 0U)
    {
        ...
        */
        ...
        {
            /* Perform the multiply-accumulates */
            /* sum += x[srcALen - srcBLen + 4] * y[3] , sum += x[srcALen -
             * srcBLen + 3] * y[2] */
            sum = __SMLALD(*__SIMD32(px)++, *__SIMD32(py)++, sum);
            /* sum += x[srcALen - srcBLen + 2] * y[1] , sum += x[srcALen -
             * srcBLen + 1] * y[0] */
            sum = __SMLALD(*__SIMD32(px)++, *__SIMD32(py)++, sum);
