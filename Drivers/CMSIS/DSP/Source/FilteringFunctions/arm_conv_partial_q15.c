    ...
    else
    {
        ...
        if (srcBLen >= 4U)
        {
            ...
            while (blkCnt > 0U)
            {
                ...
                /* Store the results in the accumulators in the destination
                 * buffer. */

#ifndef ARM_MATH_BIG_ENDIAN

                *__SIMD32(pOut)++ = __PKHBT(
                    __SSAT((acc0 >> 15), 16), __SSAT((acc1 >> 15), 16), 16);
                *__SIMD32(pOut)++ = __PKHBT(
                    __SSAT((acc2 >> 15), 16), __SSAT((acc3 >> 15), 16), 16);

#else

                *__SIMD32(pOut)++ = __PKHBT(
                    __SSAT((acc1 >> 15), 16), __SSAT((acc0 >> 15), 16), 16);
                *__SIMD32(pOut)++ = __PKHBT(
                    __SSAT((acc3 >> 15), 16), __SSAT((acc2 >> 15), 16), 16);
         ...
         */
        /* Second part of this stage computes the MAC operations less than or
         * equal to 4 */
        ...
        while ((j > 0U) && (blockSize3 > 0))
        {
            ...
            while (k > 0U)
            {
                /* x[srcALen - srcBLen + 1], x[srcALen - srcBLen + 2] are
                 * multiplied with y[srcBLen - 1], y[srcBLen - 2] respectively
                 */
                sum = __SMLALDX(*__SIMD32(px)++, *__SIMD32(py)--, sum);
                /* x[srcALen - srcBLen + 3], x[srcALen - srcBLen + 4] are
                 * multiplied with y[srcBLen - 3], y[srcBLen - 4] respectively
                 */
                sum = __SMLALDX(*__SIMD32(px)++, *__SIMD32(py)--, sum);
