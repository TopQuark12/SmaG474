                 ...
                 uint32_t blockSize)
{
     ...
     * * x[2] acc3 =  b[numTaps-1] * x[n-numTaps+2] + b[numTaps-2] *
    ...
    while (blkCnt > 0U)
    {
        ...
        *__SIMD32(pStateCurnt)++ = *__SIMD32(pSrc)++;
        ...
        /* The results in the 4 accumulators are in 2.30 format.  Convert
         *to 1.15 with saturation.
                 ...
                 uint32_t blockSize)
{
    ...
    while (blkCnt > 0U)
    {
        ...
        *pStateCurnt++ = *pSrc++;
        ...
        /* Typecast q15_t pointer to q31_t pointer for coefficient reading in
         * q31_t */
        pb = pCoeffs;
                 ...
                 uint32_t blockSize)
{
    ...
    while (blkCnt > 0U)
    {
        ...
        *pStateCurnt++ = *pSrc++;
        ...
        do
        {
            /* acc =  b[numTaps-1] * x[n-numTaps-1] + b[numTaps-2] *
             * x[n-numTaps-2] + b[numTaps-3] * x[n-numTaps-3] +...+ b[0] * x[0]
             */
            acc += (q31_t)*px++ * *pb++;
            tapCnt--;
        } while (tapCnt > 0U);
