...
void arm_cfft_radix4_q31(const arm_cfft_radix4_instance_q31 *S, q31_t *pSrc)
{
    if (S->ifftFlag == 1U)
    {
        ...
        arm_radix4_butterfly_inverse_q31(
            pSrc, S->fftLen, S->pTwiddle, S->twidCoefModifier);
    }
    else
    {
        /* Complex FFT radix-4 */
                              ...
                              uint32_t twidCoefModifier)
{
    ...
    for (k = fftLen / 4U; k > 4U; k >>= 2U)
    {
        ...
        for (j = 0U; j <= (n2 - 1U); j++)
        {
            ...
            {
                /*  index calculation for the input as, */
                /*  pSrc[i0 + 0], pSrc[i0 + fftLen/4], pSrc[i0 + fftLen/2U],
                 * pSrc[i0 + 3fftLen/4] */
                i1 = i0 + n2;
                i2 = i1 + n2;
                i3 = i2 + n2;
     ...
     * stages */
    /* data is in 9.23(q23) format for the 256 point as there are 2 middle
     * stages */
    /* data is in 7.25(q25) format for the 64 point as there are 1 middle stage
     */
    /* data is in 5.27(q27) format for the 16 point as there are no middle
     * stages */
