                              ...
                              uint32_t twidCoefModifier)
{
    ...
    do
    {
        ...
        // in = ((int16_t) (T & 0xFFFF)) >> 2;       // alternative code that
        // takes 3 cycles T = ((T >> 2) & 0xFFFF0000) | (in & 0xFFFF);
    ...
    for (k = fftLen / 4U; k > 4U; k >>= 2U)
    {
        ...
        for (j = 0U; j <= (n2 - 1U); j++)
        {
            ...
            {
                /*  index calculation for the input as, */
                /*  pSrc16[i0 + 0], pSrc16[i0 + fftLen/4], pSrc16[i0 +
                 * fftLen/2], pSrc16[i0 + 3fftLen/4] */
                i1 = i0 + n2;
                i2 = i1 + n2;
                i3 = i2 + n2;
