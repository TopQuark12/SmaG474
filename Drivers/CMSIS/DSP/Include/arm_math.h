 ...
 */
    ...
    {
        /*  q31_t r,     s;  without initialisation 'arm_offset_q15 test' fails
         * but 'intrinsic' tests pass! for armCC */
        q31_t r = 0, s = 0;
    ...
    CMSIS_INLINE __STATIC_INLINE uint64_t __SMLALDX(uint32_t x,
                                                    ...
                                                    uint64_t sum)
    {
        /*  return (sum + ((q15_t) (x >> 16) * (q15_t) y)) + ((q15_t) x *
         * (q15_t) (y >> 16)); */
    ...
    typedef struct
    {
        ...
        q15_t *pCoeffs; /**< Points to the array of coefficients.  The array is
    ...
    typedef struct
    {
        ...
        q31_t *pState;      /**< Points to the array of state coefficients.  The
                               array is of length 4*numStages. */
        q31_t *pCoeffs; /**< Points to the array of coefficients.  The array is
                           of length 5*numStages. */
    ...
    /**
     * @brief Fast but less precise processing function for the Q15 Biquad
     * cascade filter for Cortex-M3 and Cortex-M4.
     * @param[in]  S          points to an instance of the Q15 Biquad cascade
     * structure.
     * @param[in]  pSrc       points to the block of input data.
     * @param[out] pDst       points to the block of output data.
    ...
    /**
     * @brief Fast but less precise processing function for the Q31 Biquad
     * cascade filter for Cortex-M3 and Cortex-M4.
     * @param[in]  S          points to an instance of the Q31 Biquad cascade
     * structure.
     * @param[in]  pSrc       points to the block of input data.
     ...
     * <code>ARM_MATH_SIZE_MISMATCH</code> or <code>ARM_MATH_SUCCESS</code>
    ...
    /**
     * @brief Q15 matrix multiplication (fast variant) for Cortex-M3 and
     * Cortex-M4
     * @param[in]  pSrcA   points to the first input matrix structure
     * @param[in]  pSrcB   points to the second input matrix structure
     * @param[out] pDst    points to output matrix structure
    ...
    /**
     * @brief Q31 matrix multiplication (fast variant) for Cortex-M3 and
     * Cortex-M4
     * @param[in]  pSrcA  points to the first input matrix structure
     * @param[in]  pSrcB  points to the second input matrix structure
     * @param[out] pDst   points to output matrix structure
     ...
     * @param[in,out] S               points to an instance of the PID
     * structure.
    ...
    typedef struct
    {
        ...
        uint16_t
            bitRevFactor; /**< bit reversal modifier that supports different
     ...
     * structure.
     * @param[in]     S_RFFT     points to an instance of Q31 RFFT/RIFFT
     * structure
     * @param[in]     S_CFFT     points to an instance of Q31 CFFT/CIFFT
     * structure
     * @param[in]     N          length of the DCT4.
     * @param[in]     Nby2       half of the length of the DCT4.
     * @param[in]     normalize  normalizing factor.
     * @return      arm_status function returns ARM_MATH_SUCCESS if
     * initialization is successful or ARM_MATH_ARGUMENT_ERROR if <code>N</code>
     ...
     * @param[in,out] S          points to an instance of the Q15 FIR
     ...
     * @param[in,out] S          points to an instance of the Q31 FIR
     * interpolator structure.
     * @param[in]     L          upsample factor.
     * @param[in]     numTaps    number of filter coefficients in the filter.
     * @param[in]     pCoeffs    points to the filter coefficient buffer.
    ...
    /**
     * @brief Instance structure for the high precision Q31 Biquad cascade
     * filter.
     */
    typedef struct
    {
    ...
    /**
     * @brief Instance structure for the floating-point transposed direct form
    ...
    /**
     * @brief Processing function for the floating-point transposed direct form
     * II Biquad cascade filter.
     * @param[in]  S          points to an instance of the filter data
     * structure.
     * @param[in]  pSrc       points to the block of input data.
     * @param[out] pDst       points to the block of output data
     * @param[in]  blockSize  number of samples to process.
     ...
     * @param[in] pCoeffs    points to the coefficient buffer.  The array is of
     * length numStages.
     * @param[in] pState     points to the state buffer.   The array is of
     * length numStages.
     */
    ...
    typedef struct
    {
        ...
        q15_t *pvCoeffs; /**< points to the ladder coefficient array. The array
    ...
    typedef struct
    {
        ...
        q31_t *pkCoeffs; /**< points to the reflection coefficient array. The
                            array is of length numStages. */
        q31_t *pvCoeffs; /**< points to the ladder coefficient array. The array
                            is of length numStages+1. */
     ...
     * array is of length numStages.
     * @param[in] pvCoeffs   points to the ladder coefficient buffer.  The array
     * is of length numStages+1.
     * @param[in] pState     points to the state buffer.  The array is of length
    ...
    typedef struct
    {
        ...
        float32_t *pState;   /**< points to the state buffer array. The array is
                                of length maxDelay+blockSize-1. */
        float32_t *pCoeffs;  /**< points to the coefficient array. The array is
                                of length numTaps.*/
        uint16_t
     ...
     *
     * \par
     * where \c Kp is proportional constant, \c Ki is Integral constant and \c
     * Kd is Derivative constant
     *
     * \par
     ...
     * together in an instance data structure. A separate instance structure
     * must be defined for each PID Controller. There are separate instance
     * structure declarations for each of the 3 supported data types.
     *
     * \par Reset Functions
     * There is also an associated reset function for each data type which
     * clears the state array.
     *
     ...
     * clip. In order to avoid overflows completely the input signal must be
     * scaled down by 2 bits as there are four additions. After all
     * multiply-accumulates are performed, the 2.62 accumulator is truncated
     * to 1.32 format and then saturated to 1.31 format.
     */
     ...
     * internal overflow with this approach and the full precision of
     * intermediate multiplications is preserved. After all additions have been
     * performed, the accumulator is truncated to 34.15 format by discarding low
     * 15 bits. Lastly, the accumulator is saturated to yield a result in 1.15
     * format.
     */
     ...
     * @param[in]  src   points to the instance of the input floating-point
     * matrix structure.
     * @param[out] dst   points to the instance of the output floating-point
     * matrix structure.
     * @return The function returns ARM_MATH_SIZE_MISMATCH, if the dimensions do
     * not match. If the input matrix is singular (does not have an inverse),
     * then the algorithm terminates and returns error status ARM_MATH_SINGULAR.
     */
     ...
     * <code>Ia</code> as shown in the figure below \image html clarke.gif
     * Stator current space vector and its components in (a,b). and <code>Ia +
     * Ib + Ic = 0</code>, in this condition <code>Ialpha</code> and
     * <code>Ibeta</code> can be calculated using only <code>Ia</code> and
     * <code>Ib</code>.
     *
     ...
     * html clarkeFormula.gif where <code>Ia</code> and <code>Ib</code> are the
     * instantaneous stator phases and <code>pIalpha</code> and
     * <code>pIbeta</code> are the two coordinates of time invariant vector.
     * \par Fixed-Point Behavior
     * Care must be taken when using the Q31 version of the Clarke transform.
     * In particular, the overflow and saturation behavior of the accumulator
     * used must be considered. Refer to the function specific documentation
    ...
    CMSIS_INLINE __STATIC_INLINE void arm_clarke_q31(q31_t Ia,
                                                     ...
                                                     q31_t *pIbeta)
    {
        q31_t product1, product2; /* Temporary variables used to store
     ...
     * Inverse Clarke transform converts the two-coordinate time invariant
     * vector into instantaneous stator phases.
     *
     * The function operates on a single sample of data and each call to the
     * function returns the processed output. The library provides separate
     * functions for Q31 and floating-point data types. \par Algorithm \image
     * html clarkeInvFormula.gif where <code>pIa</code> and <code>pIb</code> are
     * the instantaneous stator phases and <code>Ialpha</code> and
     * <code>Ibeta</code> are the two coordinates of time invariant vector. \par
                                                         ...
                                                         float32_t *pIb)
    {
        ...
        *pIa = Ialpha;
        ...
        /* Calculating pIb from Ialpha and Ibeta by equation pIb = -(1/2) *
     ...
     *
     * Forward Park transform converts the input two-coordinate vector to flux
     * and torque components. The Park transform can be used to realize the
     * transformation of the <code>Ialpha</code> and the <code>Ibeta</code>
     * currents from the stationary to the moving reference frame and control
     * the spatial relationship between the stator vector current and rotor flux
     * vector. If we consider the d axis aligned with the rotor flux, the
     * diagram below shows the current vector and the relationship from the two
     * reference frames: \image html park.gif "Stator current space vector and
     * its component in (a,b) and in the d,q rotating reference frame"
     *
     * The function operates on a single sample of data and each call to the
     * function returns the processed output. The library provides separate
     * functions for Q31 and floating-point data types. \par Algorithm \image
     * html parkFormula.gif where <code>Ialpha</code> and <code>Ibeta</code> are
     * the stator vector components, <code>pId</code> and <code>pIq</code> are
     * rotor vector components and <code>cosVal</code> and <code>sinVal</code>
     * are the cosine and sine values of theta (rotor flux position). \par
     * Fixed-Point Behavior Care must be taken when using the Q31 version of the
     * Park transform. In particular, the overflow and saturation behavior of
     * the accumulator used must be considered. Refer to the function specific
     * documentation below for usage guidelines.
     */
                                                   ...
                                                   q31_t cosVal)
    {
        q31_t product1, product2; /* Temporary variables used to store
                                     intermediate results */
        q31_t product3, product4; /* Temporary variables used to store
                                     intermediate results */
     ...
     * function returns the processed output. The library provides separate
     * functions for Q31 and floating-point data types. \par Algorithm \image
     * html parkInvFormula.gif where <code>pIalpha</code> and
     * <code>pIbeta</code> are the stator vector components, <code>Id</code> and
     * <code>Iq</code> are rotor vector components and <code>cosVal</code> and
     * <code>sinVal</code> are the cosine and sine values of theta (rotor flux
     * position). \par Fixed-Point Behavior Care must be taken when using the
     * Q31 version of the Park transform. In particular, the overflow and
     * saturation behavior of the accumulator used must be considered. Refer to
                                                       ...
                                                       float32_t cosVal)
    {
        ...
        *pIalpha = Id * cosVal - Iq * sinVal;
        ...
        /* Calculate pIbeta using the equation, pIbeta = Id * sinVal + Iq *
         * cosVal */
     ...
     * polynomials. Linear interpolation works by effectively drawing a straight
     * line between two neighboring samples and returning the appropriate point
     * along that line
     *
     * \par
     ...
     * for Q7, Q15, Q31, and floating-point data types.  The functions operate
     * on a single sample of data and each call to the function returns a single
     ...
     * Function.
     * @param[in,out] S  is an instance of the floating-point Linear
    ...
    arm_linear_interp_f32(arm_linear_interp_instance_f32 *S, float32_t x)
    {
        ...
        {
            /* Iniatilize output for above specified range as last output value
             * of table */
                                                           ...
                                                           uint32_t blockSize)
    {
        ...
        while (i > 0U)
        {
            /* copy the sample from the circular buffer to the destination
             * buffer */
     ...
     * @param[in]  index      is the array index of the minimum value in the
     * input buffer.
     */
     ...
     * @param[in]  pIndex     is the array index of the minimum value in the
     * input buffer.
     */
     ...
     * @param[in]  blockSize  is the number of samples to process
     * @param[out] pResult    is output pointer
     * @param[out] pIndex     is the array index of the minimum value in the
     * input buffer.
     */
     ...
     *
     * Bilinear interpolation is an extension of linear interpolation applied to
     * a two dimensional grid. The underlying function <code>f(x, y)</code> is
     ...
     * linear interpolation, first in the x-dimension and then in the
     * y-dimension. Bilinear interpolation is often used in image processing to
     * rescale images. The CMSIS DSP library provides bilinear interpolation
     * functions for Q7, Q15, Q31, and floating-point data types.
     *
     * <b>Algorithm</b>
     * \par
     ...
     * <code>numRows*numCols</code> values. The data table <code>pTable</code>
     * is organized in row order and the supplied data values fall on integer
     ...
     * </pre>
     * Note that the coordinates (x, y) contain integer and fractional
     * components. The integer components specify which portion of the table to
     * use while the fractional components control the interpolation processor.
     *
     * \par
     * if (x,y) are outside of the table boundary, Bilinear interpolation
     * returns zero output.
     */
        ...
        arm_bilinear_interp_instance_q15 *S, q31_t X, q31_t Y)
    {
        ...
        /* Calculation of x1 * (1-xfract ) * (1-yfract) and acc is in 13.51
         * format */
    ...
    arm_bilinear_interp_q7(arm_bilinear_interp_instance_q7 *S, q31_t X, q31_t Y)
    {
        ...
        /* Calculation of x1 * (1-xfract ) * (1-yfract) and acc is in 16.47
         * format */
