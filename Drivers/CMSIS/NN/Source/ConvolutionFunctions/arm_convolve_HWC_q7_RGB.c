                                   ...
                                   q7_t *bufferB)
{
    ...
    /*
     *  Here we use bufferA as q15_t internally as computation are done with
     * q15_t level im2col are done to output in q15_t format from q7_t input
     */
    ...
    for (i_out_y = 0; i_out_y < dim_im_out; i_out_y++)
    {
        for (i_out_x = 0; i_out_x < dim_im_out; i_out_x++)
        {
                 ...
                 i_ker_y++)
            {
                     ...
                     i_ker_x++)
                {
                    if (i_ker_y < 0 || i_ker_y >= dim_im_in || i_ker_x < 0 ||
                        i_ker_x >= dim_im_in)
                    {
                        /* Equivalent to arm_fill_q15(0, pBuffer, ch_im_in) with
                         * assumption: ch_im_in = 3 */
                    ...
                    else
                    {
                         ...
                         * (q7_t*)Im_in+(i_ker_y*dim_im_in+i_ker_x)*3, pBuffer,
                         * 3);
                         */

                        const q7_t *pPixel =
                            Im_in + (i_ker_y * dim_im_in + i_ker_x) * 3;
            ...
            {
                pOut =
                    arm_nn_mat_mult_kernel_q7_q15(wt,
                                                  bufferA,
                                                  ch_im_out,
